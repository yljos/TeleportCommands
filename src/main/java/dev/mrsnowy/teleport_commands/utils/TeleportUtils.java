...
public class TeleportUtils {
    public static void teleportPlayer(ServerPlayerEntity player, ServerWorld world, Vec3d coords) {
        // 传送前效果
        // world.spawnParticles(ParticleTypes.PORTAL, player.getX(), player.getY()+0.5, player.getZ(), 40, 0.0D, 1.0D, 0.0D, 0.03);
        ...
        // 传送后效果
        // world.spawnParticles(ParticleTypes.PORTAL, player.getX(), player.getY()+0.5, player.getZ(), 45, 0.0D, 0.0D, 0.0D, 0.03);
        world.playSound(null, player.getBlockPos(), SoundEvents.ENTITY_ENDERMAN_TELEPORT, SoundCategory.PLAYERS, 1.0f, 1.0f);
    }
}